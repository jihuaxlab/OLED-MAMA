    ...
    for json_file in json_files:
        ...
        for mol_name, props in all_mol.items():
            ...
            for json_key, value in props.items():
                ...
                if '(nm)' in json_key_lower or '[nm]' in json_key_lower or ' nm' in json_key_lower or r'/nm' in json_key_lower:
                    unit = 'nm'
                elif '(%)' in json_key_lower or '[%]' in json_key_lower or ' %' in json_key_lower or r'/%' in json_key_lower:
                    unit = '%'
                else:
                    unit = None
                        ...
                        if has_priority:
                            ...
                            for i, prop_name in enumerate(matched_props):
                                ...
                                if unit == 'nm':
                                    ...
                                    print(f"⚠️ PL属性 '{prop_name}' 在 key='{json_key}' 中未识别单位，跳过值: {val}")
                                    print()
                                    continue
                                print(f"🆗️ PL属性 '{prop_name}' 在 key='{json_key}' 中, 值: {val}")
