...
## Pipeline Steps
The framework processes PDFs in four sequential stages:
Preprocessing with YOLO

快速start
直接运行 ./run_pipeline.sh <raw_pdf_dir> <yolo_weight> <yolo_project> [output_dir] [gpu_id]
...
下游MLs训练
我们在MLs文件夹中提供了论文中展示的提取并且校验过的TADF分子的6种不同的具有代表性的properties 的data 数据
